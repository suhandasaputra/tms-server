...
import com.boserverpos.scheduler.InitParameterSingleton;
...
public class InitConfiguration extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            /* TODO output your page here
             out.println("<html>");
             out.println("<head>");
             out.println("<title>Servlet InitConfiguration2</title>");  
             out.println("</head>");
             out.println("<body>");
             out.println("<h1>Servlet InitConfiguration2 at " + request.getContextPath () + "</h1>");
             out.println("</body>");
             out.println("</html>");
             */
        } finally {
    ...
    @Override
    public void destroy() {
        System.out.println("=-=-=-=-=-=-=-=-=-=-= InitConfiguration destroy mancap");
        log.info("=-=-=-=-=-=-=-=-=-=-= InitConfiguration destroy mancap");
        InitParameterSingleton.getInstance().setStatusThread(false);
//        Thread.currentThread().interrupt();
//        ReportScheduler.interrupted();
